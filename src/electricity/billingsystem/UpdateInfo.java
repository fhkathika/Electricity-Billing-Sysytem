...
public class UpdateInfo extends JFrame implements ActionListener {
    ...
    UpdateInfo(String meter){
        ...
        try{
            Conn con=new Conn();
           ResultSet rs=con.s.executeQuery("select * from customer where='"+meter+"'");
           while(rs.next()){
        ...
        if(ae.getSource()==b1){
            String s1=l11.getText();
             String s2=l11.getText();
              String s3=l11.getText();
               String s4=l11.getText();
                String s5=l11.getText();
                 String s6=l11.getText();
                  String s7=l11.getText();
                  try{
                      Conn con=new Conn();
                      con.s.executeQuery("update customer set address='"+s3+"',city='"+s4+"',state='"+s5+"',email='"+s6+"',phone='"+s7+"' where meter='"+meter+"'");
                  }catch(Exception e){}
